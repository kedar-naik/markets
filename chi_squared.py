...
def chi_squared_pdf(x, k):
    ...
    for i in range(len(x)):
        ...
        if x[i] > 0:
            ...
            pdf[i] = x[i]**(k/2 - 1)*np.exp(-x[i]/2)/(2**(k/2)*gamma(k/2))
        else:
...
def main():
    ...
    plt.text(ellipse_rhs, confidence_prob, '$'+str(100*confidence_prob)+'\%$')
    plt.xlabel('$x$', fontsize=the_fontsize)
    ...
    # plotting
    for i in range(len(ks)):
        plt.plot(x, cdfs_scipy[i], color=colors[i], label='$k='+str(ks[i])+'$')
    # plot the point that was found using fsolve
    plt.plot(ellipse_rhs, confidence_prob, 'k.')
    plt.text(ellipse_rhs, confidence_prob, '$'+str(100*confidence_prob)+'\%$')
    plt.xlabel('$x$', fontsize=the_fontsize)
