    ...
    for k in range(T - 1):
        ...
        for i in range(N):
            for j in range(N):
                ...
                xi[i, j, k] = alpha[i, k] * A[i, j] * B[j, o_next_index] * \
                              beta[j, k + 1]
    ...
    for i in range(N):
        for j in range(N):
            ...
            ln_A_new[i, j] = max_ln_xi + \
                             np.log(np.sum(
                                 np.exp(ln_xi[i, j, :-1] - max_ln_xi))) - \
                             max_ln_gamma - \
                             np.log(np.sum(
                                 np.exp(ln_gamma[i, :-1] - max_ln_gamma)))
...
def implied_emission_matrix_log(ln_gamma, O, V):
    ...
    for j in range(K):
        ...
        if not np.sum(column_indices):
            ...
            row_maxes = np.max(ln_gamma[:, column_indices], axis=1).reshape(
                (N, 1))
            ...
            ln_B_new[:, j] = np.squeeze(row_maxes + \
                                        np.log(np.sum(np.exp(
                                            ln_gamma[:, column_indices] - \
                                            row_maxes), axis=1)).reshape(
                                            (N, 1)))
    ...
    ln_P_in_i = row_maxes + \
                np.log(np.sum(np.exp(ln_gamma - row_maxes), axis=1)).reshape(
                    (N, 1))
...
def implied_emission_matrix(ln_gamma, O, V):
    ...
    for i in range(N):
        ...
        for j in range(K):
            ...
            for t in range(T):
                if O[t] == V[j]:
                    ln_row_sum_chosen = ln_sum(ln_row_sum_chosen,
                                               ln_gamma[i, t])
...
def baum_welch_basic(O, n_hidden_states, V, A_init=np.zeros(0),
    ...
    for iteration in range(n_max_iterations):
        ...
        P_O, alpha_old = forward_basic(O, A_old, B_old, pi_old, V,
                                       return_matrix=True)
        P_O, beta_old = backward_basic(O, A_old, B_old, pi_old, V,
                                       return_matrix=True)
        # expectation step: compute the expected state transition count matrix
        xi = expected_transition_counts_basic(O, A_old, B_old, alpha_old,
                                              beta_old, V)
        ...
        converged_A = check_cauchy_convergence(A_old, A_new,
                                               cauchy_criterion_A)
        converged_B = check_cauchy_convergence(B_old, B_new,
                                               cauchy_criterion_B)
    ...
    if not converged_A or not converged_B:
        ...
        print('\t\t  - convergence criterion:     ' + str(
            cauchy_criterion_B) + '\n')
...
def baum_welch(O, n_hidden_states, V, A_init=np.zeros(0), B_init=np.zeros(0),
    ...
    for iteration in range(n_max_iterations):
        ...
        ln_P_O, ln_alpha_old = forward(O, A_old, B_old, pi_old, V,
                                       return_matrix=True)
        ln_P_O, ln_beta_old = backward(O, A_old, B_old, pi_old, V,
                                       return_matrix=True)
        # expectation step: compute the log expected-state-transition-count matrix
        ln_xi = expected_transition_counts(O, A_old, B_old, ln_alpha_old,
                                           ln_beta_old, V)
...
def hidden_sequence_log_likelihood(H, A, pi, Q):
    ...
    for i in range(len(H) - 1):
        ...
        ln_P_current_to_next = np.log(
            A[state_indices[i], state_indices[i + 1]])
        # multiply it to the running probabilty value
...
print('\n\t\t --> P(best_path_estimate|O) = %.6f' % extended_exp(
    ln_P_best_path_est))
print('\n\t' + 70 * '-')
...
print_matrix(np.array(list(zip(Q_unknown, new_state_names))),
             name='  replace w/')
best_path_renamed = replace_hidden_state_names(best_path_est, Q_unknown,
                                               new_state_names)
...
print_matrix(expected_persistences, name='expected state persistences',
             n_indents=3)
print('\n\tOR')
...
print('\n\tchance of getting', str(round(100 * path_accuracy, 2)),
      '% accuracy ' + \
